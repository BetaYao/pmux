//! ScreenRuntime: wraps LocalPtyRuntime-style PTY but uses GNU screen for session persistence.
//!
//! Spawns `screen -S pmux-xxx $SHELL` in a PTY. Screen creates a persistent session.
...
/// Screen-backed PTY runtime. Same structure as DtachRuntime but spawns via GNU screen.
...
impl ScreenRuntime {
    /// Create a new ScreenRuntime by spawning `screen -S pmux-xxx $SHELL` in a PTY.
    pub fn new(worktree_path: &Path, cols: u16, rows: u16) -> Result<Self, RuntimeError> {
        ...
        cmd.args(["-S", &session_name, &shell]);
        cmd.cwd(worktree_path);

        let child: Box<dyn portable_pty::Child + Send + Sync> = pair
            .slave
            .spawn_command(cmd)
            .map_err(|e| RuntimeError::Backend(format!("screen spawn: {}", e)))?;
        ...
        let pane_id = format!("screen:{}", worktree_path.display());
...
impl AgentRuntime for ScreenRuntime {
    ...
    fn list_panes(&self, agent_id: &AgentId) -> Vec<PaneId> {
        ...
        if agent_id.is_empty()
            ...
            || agent_id == &format!("screen:{}", path_str)
        {
    ...
    fn split_pane(&self, _pane_id: &PaneId, _vertical: bool) -> Result<PaneId, RuntimeError> {
        Err(RuntimeError::Backend(
            "split pane not implemented in ScreenRuntime".to_string(),
        ))
    }
    ...
    fn open_diff(
        ...
        Err(RuntimeError::Backend(
            "open_diff not implemented in ScreenRuntime".to_string(),
        ))
    }

    fn open_review(&self, _worktree: &Path) -> Result<String, RuntimeError> {
        Err(RuntimeError::Backend(
            "open_review not implemented in ScreenRuntime".to_string(),
        ))
    }
...
mod tests {
    ...
    #[test]
    fn test_screen_runtime_backend_type() {
        // Don't actually create a ScreenRuntime as it requires screen installed
